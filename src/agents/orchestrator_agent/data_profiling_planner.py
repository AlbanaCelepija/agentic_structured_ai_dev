...
# ---------- Configuration ----------
MODEL = os.getenv("MODEL", "gpt-4o")  # "llama3-70b" change to model name provided by your host
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
OPENAI_API_BASE = os.getenv(
    "OPENAI_API_BASE"
)  # optional: e.g. "https://api.groq.com/openai/v1"
...
def call_planner_model(user_query: str, data_preview: str) -> Dict[str, Any]:
    ...
    messages=[
        {
            "role": "system",
            "content": "You are a strict JSON-output planner. Remove JSON comments. Do not output anything except the JSON object. Only output a single valid JSON object (and nothing else).",
        },
    ...
    try:
        ...
        raise ValueError(
            f"Planner response not valid JSON. Raw response:\n{text}\nError: {e}"
        )
    return plan_json
...
def execute_step(
    df: pd.DataFrame, step: Dict[str, Any], dataset_name: str
...
def run_planner_and_execute(csv_path: str, user_query: str):
    ...
    for step in steps:
        ...
        results.append(meta)
        print(" ->", meta.get("note", meta.get("action")))

    # Save results summary
    with open(f"{dataset_name}_plan_results.json", "w", encoding="utf-8") as f:
        json.dump(
            {"user_query": user_query, "steps": steps, "results": results},
            f,
            indent=2,
            ensure_ascii=False,
        )
