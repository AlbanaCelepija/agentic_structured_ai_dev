...
async def process_query(query: str, tools):
    ...
    model = ChatOpenAI(model_name="gpt-4o", temperature=0.7).bind_tools(tools)
    ...
    opik_tracer = OpikTracer(
        graph=graph.get_graph(xray=True), tags=["langchain", "mcp"]
    )
    config = {
...
async def main():
    server_params = StdioServerParameters(
        command="python",
        args=["../mcp_servers/mcp_server_research.py"],  # Update this path
    )
